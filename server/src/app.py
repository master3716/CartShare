...
from src.routes.notification_routes import create_notification_blueprint
from src.routes.collection_routes import create_collection_blueprint
...
def create_app() -> Flask:
    ...
    if MONGO_URI:
        ...
        from src.repositories.mongo_notification_repository import MongoNotificationRepository
        from src.repositories.mongo_collection_repository import MongoCollectionRepository
        ...
        notification_repo = MongoNotificationRepository(MONGO_URI)
        collection_repo = MongoCollectionRepository(MONGO_URI)
    else:
        ...
        notification_repo = NotificationRepository(DATABASE_PATH)
        collection_repo = CollectionRepository(DATABASE_PATH)
    ...
    friend_service = FriendService(user_repo, purchase_repo, notification_service)
    comment_service = CommentService(comment_repo, notification_service, purchase_repo)
    saved_item_service = SavedItemService(saved_item_repo, purchase_repo)
    collection_service = CollectionService(collection_repo, user_repo, purchase_repo)
    ...
    purchase_bp = create_purchase_blueprint(purchase_service, user_service, user_repo, notification_service)
    ...
    notification_bp = create_notification_blueprint(notification_service, user_repo)
    collection_bp = create_collection_blueprint(collection_service, user_repo)
