...
class CommentService:
    ...
    def add_comment(self, purchase_id: str, user_id: str, username: str, text: str) -> Comment:
        if not text or not text.strip():
            raise ValueError("Comment text is required.")
        if len(text.strip()) > 500:
            raise ValueError("Comment must be 500 characters or less.")
        comment = Comment(purchase_id=purchase_id, user_id=user_id, username=username, text=text.strip())
